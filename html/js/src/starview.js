...
(function(window) {
    ...
    var main = function(spec, my) {
        ...
        that.tick = function() {
			...
			var sdat = stars_data[i];
                        stars[i].onClick = function(evt){
				s.dialog.open(sdat.starid, sdat.name, sdat.vmag, 32);
			}
    ...
    $(window).bind('space_geosocial_ready', function(){
        ...
        s.get_near_stars(function(stars_db_data){
            ready_db_flag = 1;
            checkready();
        }, screen_azimuth, screen_elevation, ERROR);
    });
})(window);
